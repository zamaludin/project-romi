...
<section id="main-content" class="home">
	<div class="widget-title">
		<h4><?=$title;?> <i class="fa fa-newspaper-o"></i></h4>
	</div>
			...
			<li class="clearfix">
				<?php if ($this->uri->segment(2) == 'post' || $this->uri->segment(2) == 'category') {?>
					<?php if ($row->post_image != NULL && file_exists('assets/post/thumb/' . $row->post_image)) {?>
						<img src="<?=base_url('assets/post/thumb/' . $row->post_image);?>" alt="<?=$row->post_title;?>"/>
					<?php } else {?>
						<img src="<?=base_url('assets/190x190.gif');?>" alt="<?=$row->post_title;?>"/>
					<?php } ?>
				<?php } else if ($this->uri->segment(2) == 'archive') {?>
					<?php if ($row->post_image != NULL && file_exists('assets/post/thumb/' . $row->post_image)) {?>
						<img width="70" src="<?=base_url('assets/post/thumb/' . $row->post_image);?>" alt="<?=$row->post_title;?>"/>
					<?php } else {?>
						<img src="<?=base_url('assets/190x190.gif');?>" alt="<?=$row->post_title;?>"/>
					<?php } ?>
				<?php } else if ($this->uri->segment(2) == 'pengumuman') {?>
					<?php if ($row->post_image != NULL && file_exists('assets/post/' . $row->post_image)) {?>
						<img src="<?=base_url('assets/post/' . $row->post_image);?>" alt="<?=$row->post_title;?>"/>
					<?php } else {?>
						<img src="<?=base_url('assets/190x190.gif');?>" alt="<?=$row->post_title;?>"/>
					<?php } ?>
				<?php } ?>
				...
				<div class="metadata"><i class="fa fa-calendar"></i> <?=substr($row->post_date, 8, 2);?> <?=bulan(substr($row->post_date, 5, 2), 'S');?> <?=substr($row->post_date, 0, 4);?> <i class="fa fa-user"></i> <?=$row->display_name;?></div>
				<?php if ($this->uri->segment(2) == 'post' || $this->uri->segment(2) == 'category') {?>
