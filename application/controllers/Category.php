...
class Category extends MY_Controller {
	...
	public function index() {
		...
		$this->data['posts'] = $this->data['categories'] = TRUE;
		$this->data['alert'] = $this->session->flashdata('alert');
		$this->data['query'] = $this->db->order_by('category', 'ASC')->get($this->table, $config['per_page'], $this->uri->segment(3));
		$this->data['content'] = 'category/read';
	...
	public function create() {
		if ($_POST) {
			...
			$this->data['content'] = 'category/create';
			$this->load->view('backend/index', $this->data);
		}
	}
	...
	private function is_used($category_id) {
		return $this->db
			...
			->count_all_results('posts') > 0 ? true : false;
	}
}
