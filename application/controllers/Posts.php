...
class Posts extends MY_Controller {
   ...
   public function index() {
      ...
      $config['total_rows'] = $this->db->where('post_type', 'post')->count_all_results($this->table);
      $config['per_page'] = 10;
      ...
      $this->data['query'] = $this->db
         ->select('p.post_id, p.post_title, p.post_date, c.category, u.display_name')
   ...
   public function create() {
      if ($_POST) {
         ...
         $this->data['posts'] = $this->data['add_post'] = TRUE;
         $this->data['alert'] = $this->session->flashdata('alert');
         $this->data['category'] = $this->m_global->dropdown('category_id', 'category', 'category');
         $this->data['query'] = FALSE;
         $this->data['content'] = 'posts/create';
         $this->load->view('backend/index', $this->data);
      }
   }
   ...
   private function resize_photo($photo) {
      ...
      $config['width'] = $img['config']['posts']['thumb']['width'];
      ...
      $config2['width'] = $img['config']['posts']['width'];
      $config2['height'] = $img['config']['posts']['width'];
