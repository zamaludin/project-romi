...
class Registration extends MY_Controller {
   ...
   public function index() {
      ...
      $this->form_validation->set_rules('telp_ortu', 'Telepon Orang Tua', 'trim|numeric');
      ...
      if ($this->form_validation->run() == false) {
         ...
         $this->data['title'] = 'Penerimaan Peserta Didik Baru Tahun ' . date('Y');
         ...
         $this->data['content'] = 'themes/' . $this->setting['themes'] . '/ppdb/ppdb-registered';
         ...
         if ($photo['status'] == 'success') {
            ...
            if ($this->db->insert('siswa', $field_data)) {
               ...
               redirect('ppdb/confirmation/index/' . encode_url($this->db->insert_id()));
            } else {
               ...
               $this->session->set_flashdata('alert', $alert);
               redirect('ppdb/confirmation/index/' . encode_url($field_data['nisn']));
            }
         } else {
      ...
      if ($id && $id != 0 && ctype_digit((string) $id)) {
         ...
         if ($this->form_validation->run() == false) {
            ...
            if (empty($_FILES['file']['name'])) {
               ...
               redirect('ppdb/confirmation/index/' . encode_url($id));
            } else {
               $photo = $this->upload_photo();
               if ($photo['status'] == 'success') {
                  $field_data = $this->field_data($photo['data']['file_name']);
...
/* Location: ./application/controllers/ppdb/Registration.php */