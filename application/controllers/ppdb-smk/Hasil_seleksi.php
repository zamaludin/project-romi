...
class Hasil_seleksi extends MY_Controller {
   ...
   public function index() {
      redirect('ppdb-smk/hasil_seleksi/diterima');
   }

   public function diterima() {
      $tahun = $this->uri->segment(4) == FALSE ? $this->setting['ppdb_tahun'] : $this->uri->segment(4);
      $this->load->library('pagination');
      $config['base_url'] = site_url('ppdb-smk/hasil_seleksi/diterima/' . $tahun);
      ...
      $this->data['content'] = 'ppdb-smk/hasil_seleksi';
   ...
   public function export_excel() {
      ...
      $table .= '<td>JALUR PENDAFTARAN</td>';
      $table .= '<td>PILIHAN I</td>';
      $table .= '<td>PILIHAN II</td>';
      $table .= '<td>DITERIMA DI</td>';
      ...
      foreach ($query->result_array() as $data) {
         ...
         $table .= '<td>'.$data['jalur_pendaftaran'].'</td>';
         $table .= '<td>'.$data['pilihan_satu'].'</td>';
         $table .= '<td>'.$data['pilihan_dua'].'</td>';
         if ($data['hasil_seleksi'] == 'tidak_diterima') {
            $table .= '<td>'.'Tidak Diterima'.'</td>';
         } else {
            $table .= '<td>'.$this->m_global->get_jurusan($data['hasil_seleksi']).'</td>';
         }
...
/* Location: ./application/controllers/ppdb-smk/siswa.php */