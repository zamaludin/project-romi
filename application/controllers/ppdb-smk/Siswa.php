...
class Siswa extends MY_Controller {
   ...
   public function index() {
      ...
      $config['base_url'] = site_url('ppdb-smk/siswa/index/' . $tahun);
      ...
      $this->data['q_tahun'] = $this->db->query("SELECT LEFT(tanggal_daftar, 4) AS tahun
                                                       FROM siswa
      ...
      $this->data['content'] = 'ppdb-smk/read';
   ...
   public function update() {
      ...
      if ($_POST) {
         ...
         $this->data['content'] = 'ppdb-smk/create';
         $this->load->view('backend/index', $this->data);
      } else {
         $this->session->set_flashdata('alert', alert('error', status('404')));
         redirect('ppdb-smk/siswa');
      }
   }
   ...
   private function field_data($file = '') {
      ...
      $data['pilihan_1'] = $this->input->post('pilihan_1');
      $data['pilihan_2'] = $this->input->post('pilihan_2');
   ...
   private function validation() {
      ...
      $this->form_validation->set_rules('jalur_pendaftaran_id', 'Jalur Pendaftaran', 'trim|required');
   ...
   public function export_excel() {
      ...
      $objSheet->setCellValue('G1', 'SEKOLAH ASAL');
      $objSheet->setCellValue('H1', 'NISN');
      $objSheet->setCellValue('I1', 'NAMA CALON PESERTA DIDIK');
      ...
      $objSheet->setCellValue('N1', 'STATUS ANAK');
      $objSheet->setCellValue('O1', 'ANAK KE');
      $objSheet->setCellValue('P1', 'ALAMAT');
      $objSheet->setCellValue('Q1', 'TELEPON');
      $objSheet->setCellValue('R1', 'EMAIL');
      ...
      foreach($query->result_array() as $data) {
         ...
         $objSheet->setCellValue('E'.$cell, $data['pilihan_satu']);
         $objSheet->setCellValue('F'.$cell, $data['pilihan_dua']);
         $objSheet->setCellValue('G'.$cell, $data['sekolah_asal']);
         $objSheet->setCellValue('H'.$cell, $data['nisn']);
         $objSheet->setCellValue('I'.$cell, $data['nama']);
         $objSheet->setCellValue('J'.$cell, $data['tempat_lahir']);
         $objSheet->setCellValue('K'.$cell, indo_date($data['tanggal_lahir']));
         $objSheet->setCellValue('L'.$cell, $data['jenis_kelamin']);
         $objSheet->setCellValue('M'.$cell, $data['agama']);
         $objSheet->setCellValue('N'.$cell, $data['status_anak']);
         $objSheet->setCellValue('O'.$cell, $data['anak_ke']);
         $objSheet->setCellValue('P'.$cell, $data['alamat']);
         $objSheet->setCellValue('Q'.$cell, $data['telp_rumah']);
         $objSheet->setCellValue('R'.$cell, $data['email']);
