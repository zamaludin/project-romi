...
class Check extends MY_Controller {
   ...
   public function index() {
      ...
      if ($this->form_validation->run() == FALSE) {
         ...
         $this->data['content'] = 'themes/' . $this->setting['themes'] . '/ppdb-smk/ppdb-check';
         ...
         if ($query->num_rows() == 1) {
            $row = $query->row_array();
            if (is_numeric($row['hasil_seleksi'])) {
               $result = $this->m_global->get_jurusan($row['hasil_seleksi']);
               $alert = '<div class="alert alert-success">';
               $alert .= 'Selamat ' . $row['nama'] . ' ! Anda berhasil lolos seleksi Penerimaan Peserta Didik Baru Tahun ' . $tahun . ' ' . $this->setting['nama_sekolah'] . ' di jurusan ' . $result;
               $alert .= '</div>';
               ...
               $alert .= '</div>';
               $this->session->set_flashdata('alert', $alert);
            } else if ($row['hasil_seleksi'] == 'belum_diseleksi') {
               $alert = '<div class="alert alert-success">';
